...
if (!botToken) {
  ...
  console.error("❌ BOT_TOKEN is still set to the placeholder value. Please update it with your actual bot token.");
 process.exit(1);
} else {
...
if (!apiBaseURL) {
  ...
  console.error("❌ API_BASE_URL is still set to the placeholder value. Please update it with the actual API URL.");
  process.exit(1);
} else {

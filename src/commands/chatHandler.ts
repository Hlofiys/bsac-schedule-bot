...
chatHandler.on("message:text", async (ctx, next) => {
    ...
    if (isStudent) {
      ...
      try {
        ...
        groups.sort((a, b) => {
          ...
          if (aGroup.startsWith(searchLower) && !bGroup.startsWith(searchLower))
            return -1;
          if (bGroup.startsWith(searchLower) && !aGroup.startsWith(searchLower))
            return 1;
  ...
  if (ctx.user.state === UserState.AskingWeekGroup) {
    ...
    const mockGroups = [{ id: searchText, display: searchText }];

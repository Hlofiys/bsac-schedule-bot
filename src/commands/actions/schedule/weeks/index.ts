...
async function findEntityName(entityId: string, isGroup: boolean): Promise<string | undefined> {
    if (isGroup) {
        const groups = await scheduleApi.getGroups();
        return groups.find(g => g.id?.toString() === entityId)?.groupNumber ?? undefined;
    } else {
        const teachers = await scheduleApi.getTeachers();
        return teachers.find(t => t.id?.toString() === entityId)?.fio ?? undefined;
    }
}
...
weeksHandler.callbackQuery(/^(group_week|teacher_week)/, async (ctx) => {
    ...
    let [entityId, weekStartRaw] = args;
    ...
    if (!entityId) {
        if (isGroup && ctx.user.selectedGroup) {
            ...
            ctx.user.state = isGroup ? UserState.AskingWeekGroup : UserState.AskingWeekTeacher;
            await ctx.user.save();
    ...
    if (!weekStartRaw) {
        try {
            ...
            if (!entityName) {
                await ctx.answerCallbackQuery({ text: 'Сущность не найдена', show_alert: true });
                return;
            }
            ...
            console.error(e);
            await ctx.answerCallbackQuery({ text: '🤖 Произошла какая-то ошибка', show_alert: true });
        }
    } else {
        try {
            const entityName = await findEntityName(entityId, isGroup);
            ...
            if (!scheduleForWeek || scheduleForWeek.length === 0) {
                await ctx.answerCallbackQuery();
                return await ctx.editMessageText(`🏖️ Расписания на неделю для ${entityName} нету`);
            }

            const scheduleMessage = formatWeekSchedule(scheduleForWeek, entityName, ctx.user.subgroup);
            await ctx.answerCallbackQuery();
            await ctx.editMessageText(scheduleMessage, { parse_mode: "HTML", reply_markup: new InlineKeyboard() });
        } catch (e) {
            console.error(e);
            await ctx.answerCallbackQuery({ text: '🏖️ Расписания на неделю нету', show_alert: true });
        }
    }
});

function generateWeekButtons(entityId: string, isGroup: boolean): InlineKeyboard {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        buttons.text(weekLabel, callbackIdBuild(action, [entityId, dateToCallback(weekStart)])).row();
    }
    return buttons;
}
...
function formatWeekSchedule(scheduleForWeek: GetScheduleForOneGroup[], entityName: string, subgroup?: number): string {
    ...
    for (const day of sortedDays) {
        ...
        lessons.sort((a, b) => (a.lessonSchedule?.lessonNumber || 0) - (b.lessonSchedule?.lessonNumber || 0)).forEach(lessonWithWork => {

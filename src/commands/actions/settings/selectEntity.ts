  ...
  if (isStudent) {
    ...
    if (!groupId) {
      ...
      return await ctx.reply(
        "😵‍💫 Кажется произошла какая-то ошибка при выборе группы. Попробуй поискать новую группу, отправив её номер",
      );
    }
    ...
    if (!teacherName) {
      ...
      await ctx.user.save();
      return await ctx.reply(
        "😵‍💫 Кажется произошла какая-то ошибка при выборе преподавателя. Попробуй поискать другого",
      );
    }
    ...
    return await ctx.reply(
      `🕺 Выбран преподаватель *${teacherName.replace(/\\./g, "\\\\.")}*`,
      {
